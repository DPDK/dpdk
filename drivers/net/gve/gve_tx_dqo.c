...
gve_reset_txq_dqo(struct gve_tx_queue *txq)
{
	...
	size = txq->nb_tx_desc * sizeof(struct gve_tx_compl_desc);
	for (i = 0; i < size; i++)
		       ...
		       const struct rte_eth_txconf *conf)
{
	...
	rs_thresh = conf->tx_rs_thresh ? conf->tx_rs_thresh : GVE_DEFAULT_TX_RS_THRESH;
	...
	mz = rte_eth_dma_zone_reserve(dev, "tx_compl_ring", queue_id,
				      nb_desc * sizeof(struct gve_tx_compl_desc),
				      PAGE_SIZE, socket_id);
	if (mz == NULL) {
