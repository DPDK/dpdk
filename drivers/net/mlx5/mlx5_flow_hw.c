		     ...
		     struct rte_flow_error *error)
{
	...
	copied_items = mlx5_malloc(MLX5_MEM_ZERO, total_size, 0, rte_socket_id());
	if (!copied_items) {
		rte_flow_error_set(error, ENOMEM,
				   RTE_FLOW_ERROR_TYPE_UNSPECIFIED,
				   NULL,
				     ...
				     struct rte_flow_error *error)
{
	...
	if (action_flags & MLX5_FLOW_ACTION_RSS) {
		...
		flow = flow_nta_handle_rss(dev, attr, prepend_items, actions, rss_conf,
...
prefix_flow:
	ret = flow_hw_create_flow(dev, type, attr, prepend_items, resource.prefix.actions,
