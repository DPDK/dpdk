...
- Link speed, FEC mode and lane count are accepted
  without error but have no physical effect on the link,
  which is exclusively under PF control.
...
- Periodic MAC statistics DMA is not available;
  statistics are collected via one-shot uploads instead.
