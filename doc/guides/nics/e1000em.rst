...
DPDK provides several poll mode drivers (PMDs)
supporting Intel 1 Gigabit ethernet controller families.
These include:
...
.. note::
   ...
   The ``igb`` (and ``igc`` for 2.5G) PMDs are built from the same driver binary
   but are documented in separate sections for clarity.
