...
* Runtime (post-start) Rx/Tx queue setup can race with the hardware Tx rate limiter
  on E810 VFs and corrupt queue state.
  ...
  are rejected with ``-EBUSY`` on a running port
  for as long as queue rate limiting is active.
...
To trigger LLDP packet transmission from the VF,
set the ``packet_type`` of the mbuf to ``RTE_PTYPE_L2_ETHER_LLDP``.
